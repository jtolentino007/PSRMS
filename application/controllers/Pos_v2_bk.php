<?php
	...
	class Pos_v2 extends CORE_Controller
	{
		...
		public function addProductToInvoice()
		{
			...
			$response['pos_invoice_id'] = $pos_invoice_id;
			$response['response_rows'] = $this->response_rows($pos_invoice_id);
			$response['response_vendors'] = $this->response_vendors($pos_invoice_id);
			$response['title'] = 'Success!';
            $response['stat'] = 'success';
		...
		public function response_vendors($filter_value=null)
		{
			$m_pos_items = $this->Pos_items_model;
			return $m_pos_items->get_list(
				'pos_invoice_items.pos_invoice_id = '.$filter_value,
				'DISTINCT(products.vendor_id) vendor_id',
				array(
